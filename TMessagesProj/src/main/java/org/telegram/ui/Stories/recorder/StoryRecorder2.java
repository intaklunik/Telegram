...
public class StoryRecorder2 implements NotificationCenter.NotificationCenterDelegate {

    private final Theme.ResourcesProvider resourcesProvider = new DarkThemeResourceProvider();
    private ChatAttachAlertPhotoLayout chatAttachAlertPhotoLayout;
    ...
    private static StoryRecorder2 instance;
    private boolean wasSend;
    private long wasSendPeer = 0;
    private ClosingViewProvider closingSourceProvider;
    private Runnable closeListener;

    public static StoryRecorder2 getInstance(Activity activity, int currentAccount, ChatAttachAlertPhotoLayout chatAttachAlertPhotoLayout) {
        if (instance != null && (instance.activity != activity || instance.currentAccount != currentAccount)) {
            instance.close(false);
            instance = null;
        }
        if (instance == null) {
            instance = new StoryRecorder2(activity, currentAccount, chatAttachAlertPhotoLayout);
        }
        return instance;
    }
    ...
    public StoryRecorder2(Activity activity, int currentAccount, ChatAttachAlertPhotoLayout chatAttachAlertPhotoLayout) {
        this.activity = activity;
        this.currentAccount = currentAccount;
        this.chatAttachAlertPhotoLayout = chatAttachAlertPhotoLayout;
...
//            src.iconSize = AndroidUtilities.dp(30);
    ...
    public StoryRecorder2 whenSent(Runnable listener) {
        closeListener = listener;
        return this;
    }

    public StoryRecorder2 closeToWhenSent(ClosingViewProvider closingSourceProvider) {
    ...
    public void openBotEntry(long botId, String lang_code, StoryEntry entry, SourceView sourceView) {
        ...
        Log.d("TAG", "navigateTo(PAGE_PREVIEW1)");
...
//        privacySelectorHintOpened = false;
    ...
    public void openForward(SourceView sourceView, StoryEntry entry, long time, boolean animated) {
        ...
        Log.d("TAG", "navigateTo(PAGE_PREVIEW2)");
        navigateTo(PAGE_PREVIEW, false);
        switchToEditMode(EDIT_MODE_NONE, false);
    ...
    public void openRepost(SourceView sourceView, StoryEntry entry) {
        ...
        previewButtons.appear(true, false);
        Log.d("TAG", "navigateTo(PAGE_PREVIEW3)");
        navigateTo(PAGE_PREVIEW, false);
        switchToEditMode(EDIT_MODE_NONE, false);
        animateOpenTo(1, true, this::onOpenDone);
    ...
    //  windowView.setVisibility(View.GONE);
        ...
        isShown = false;
        //animateOpenTo(0, false, this::onCloseDone);
...
//                return r;
        ...
        protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            ...
            if (hFromW + underControls <= H - navbar) {
                ...
                previewH = H; //H - underControls - navbar - statusbar;
            ...
            containerView.measure(
                ...
                MeasureSpec.makeMeasureSpec(previewH
                       // + underControls
                        , MeasureSpec.EXACTLY)
            );
...
//                    captionEdit.editText.hideEmojiView();
        ...
        protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
            ...
            final int statusbar = insetTop;
            final int underControls = 0;//navbarContainer.getMeasuredHeight();
    ...
    private class ContainerView extends FrameLayout {
        ...
        protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
            ...
            actionBarContainer.layout(0,
                    Math.min(insetTop, AndroidUtilities.statusBarHeight), //t,
                    previewW,
                    Math.min(insetTop, AndroidUtilities.statusBarHeight) //t
                            + actionBarContainer.getMeasuredHeight()
            );
            controlContainer.layout(0,
                    getMeasuredHeight() - controlContainer.getMeasuredHeight() - insetTop - insetBottom //previewH - controlContainer.getMeasuredHeight() + 200
                    , previewW,
                    getMeasuredHeight()  - insetTop - insetBottom //previewH + 200
                    );
            navbarContainer.layout(0,
                    getMeasuredHeight() - insetTop - insetBottom, //previewH + 200,
                    previewW,
                    getMeasuredHeight() + navbarContainer.getMeasuredHeight() - insetTop - insetBottom //previewH + navbarContainer.getMeasuredHeight() + 200
            );
           // captionContainer.layout(0, 0, previewW, previewH);
            captionContainer.layout(0, 0, previewW, getMeasuredHeight() - insetTop - insetBottom);
            ...
            if (captionEdit.mentionContainer != null) {
                ...
                captionEdit.mentionContainer.layout(0, 0, previewW, getMeasuredHeight() - insetTop - insetBottom);
        ...
        protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            ...
            if (captionEdit.mentionContainer != null) {
               ...
               // measureChildExactly(captionEdit.mentionContainer, previewW, getMeasuredHeight() - insetBottom - insetTop);
            }
...
//        flashButton.setAlpha(0f);
     ...
     //   actionBarContainer.addView(flashButton, LayoutHelper.createFrame(56, 56, Gravity.TOP | Gravity.RIGHT));
        ...
        actionBarContainer.addView(dualButton, LayoutHelper.createFrame(56, 56, Gravity.TOP | Gravity.RIGHT, 0, 0, 0, 0));
        ...
        flashViews.add(flashButton);
        actionBarContainer.addView(collageButton, LayoutHelper.createFrame(56, 56, Gravity.TOP | Gravity.RIGHT, 56, 0, 0, 0));
        actionBarContainer.addView(flashButton, LayoutHelper.createFrame(56, 56, Gravity.TOP | Gravity.RIGHT, 128, 0, 0, 0));
        ...
        coverButton.setOnClickListener(v -> {
            ...
            if (outputEntry != null && !outputEntry.isEditingCover) {
                AndroidUtilities.runOnUIThread(() -> {
                    ...
                    Log.d("TAG", "navigateTo(PAGE_PREVIEW4)");
                    MediaController.PhotoEntry photoEntry = new MediaController.PhotoEntry(0, ChatAttachAlertPhotoLayout.lastImageId, 0, outputFile.getAbsolutePath(), 0, outputEntry.isVideo, outputEntry.width, outputEntry.height, 0);
                    photoEntry.canDeleteAfter = true;
                    chatAttachAlertPhotoLayout.openPhotoViewer(photoEntry, true, false);
                }, 400);
            }
        });
...
//        }
    ...
    private final RecordControl.Delegate recordControlDelegate = new RecordControl.Delegate() {
        ...
        public void onCheckClick() {
            ...
            StoryPrivacySelector.applySaved(currentAccount, outputEntry);
            Log.d("TAG", "navigateTo(PAGE_PREVIEW5)");
            MediaController.PhotoEntry photoEntry = new MediaController.PhotoEntry(0, ChatAttachAlertPhotoLayout.lastImageId, 0,
                    outputEntry.collageContent.get(0).file.getAbsolutePath(), 0, false, outputEntry.width, outputEntry.height, 0);
            photoEntry.canDeleteAfter = true;
            chatAttachAlertPhotoLayout.openPhotoViewer(photoEntry, true, false);
        }

        private void takePicture(Utilities.Callback<Runnable> done) {
            Log.d("TAG", "TakePicture_SR");
            ...
            if (!savedFromTextureView) {
                Log.d("TAG", "!savedFromTextureView");
                takingPhoto = CameraController.getInstance().takePicture(outputFile, true, cameraView.getCameraSessionObject(), (orientation) ->
                {
                    ...
                    if (collageLayoutView.hasLayout()) {
                        Log.d("TAG", "collageLayoutView.hasLayout()");
                        outputFile = null;
...
//
                        ...
                        Log.d("TAG", "else collageLayoutView.hasLayout()");
                        outputEntry = entry;
                        StoryPrivacySelector.applySaved(currentAccount, outputEntry);
                        fromGallery = false;

                        if (done != null) {
                            Log.d("TAG", "NavigateTO !null");
                            Log.d("TAG", "navigateTo(PAGE_PREVIEW6)");
                            done.run(() -> {
                                MediaController.PhotoEntry photoEntry = new MediaController.PhotoEntry(0, ChatAttachAlertPhotoLayout.lastImageId, 0, outputFile.getAbsolutePath(),
                                        orientation == -1 ? 0 : orientation,
                                        entry.isVideo, entry.width, entry.height, 0);
                                photoEntry.canDeleteAfter = true;
                                chatAttachAlertPhotoLayout.openPhotoViewer(photoEntry, true, false);
                            });
                        }
                            ...
                            Log.d("TAG", "NavigateTO null");
                            MediaController.PhotoEntry photoEntry = new MediaController.PhotoEntry(0, ChatAttachAlertPhotoLayout.lastImageId, 0, outputFile.getAbsolutePath(), orientation == -1 ? 0 : orientation, false, entry.width, entry.height, 0);
                            photoEntry.canDeleteAfter = true;

                            Log.d("TAG", "OpenPhotoViewer3");
                            chatAttachAlertPhotoLayout.openPhotoViewer(photoEntry, true, false);
                        }
                    }
                });
                ...
                Log.d("TAG", "else savedFromTextureView");
                takingPhoto = false;
                ...
                if (collageLayoutView.hasLayout()) {
                    ...
                    if (done != null) {
                        Log.d("TAG", "navigateTo(PAGE_PREVIEW7)");
                        done.run(() -> {
                                    MediaController.PhotoEntry photoEntry = new MediaController.PhotoEntry(0, ChatAttachAlertPhotoLayout.lastImageId,
                                            0, outputFile.getAbsolutePath(),
                                            0, entry.isVideo, entry.width, entry.height, 0);
                                    photoEntry.canDeleteAfter = true;
                                    chatAttachAlertPhotoLayout.openPhotoViewer(photoEntry, true, false);
                        ...
                        Log.d("TAG", "navigateTo(PAGE_PREVIEW8)");
                            MediaController.PhotoEntry photoEntry = new MediaController.PhotoEntry(0, ChatAttachAlertPhotoLayout.lastImageId,
                                    0, outputFile.getAbsolutePath(),
                                    0, entry.isVideo, entry.width, entry.height, 0);
                            photoEntry.canDeleteAfter = true;
        ...
        private void startRecording(boolean byLongPress, Runnable whenStarted) {
            ...
            CameraController.getInstance().recordVideo(cameraView.getCameraSessionObject(), outputFile, false, (thumbPath, duration) -> {
                ...
                if (collageLayoutView.hasLayout()) {
                    ...
                    navigateToPreviewWithPlayerAwait(() -> {
                        Log.d("TAG", "navigateTo(PAGE_PREVIEW9)");
                        if (outputFile == null
                               // || parentAlert.destroyed
                                || cameraView == null) {
                            return;
                        }
                       // mediaFromExternalCamera = false;
                        ...
                        MediaController.PhotoEntry photoEntry = new MediaController.PhotoEntry(0, chatAttachAlertPhotoLayout.lastImageId--, 0, outputFile.getAbsolutePath(), 0, outputEntry.isVideo, width1, height1, 0);
                        photoEntry.duration = (int) (duration / 1000f);
                        photoEntry.thumbPath = thumbPath;
                        if (
                        //        parentAlert.avatarPicker != 0 &&
        ...
        public void onVideoRecordEnd(boolean byDuration) {
            Log.d("TAG", "OnVideoRecordEnd: " + byDuration);
...
//                    setAwakeLock(false);
    ...
    public boolean onBackPressed() {
        ...
        if (captionEdit.onBackPressed()) {
            ...
            processDone();
            Log.d("TAG", "navigateTo(PAGE_PREVIEW10)");
            MediaController.PhotoEntry photoEntry = new MediaController.PhotoEntry(0, ChatAttachAlertPhotoLayout.lastImageId, 0, outputFile.getAbsolutePath(), 0, outputEntry.isVideo, outputEntry.width, outputEntry.height, 0);
            photoEntry.canDeleteAfter = true;
            chatAttachAlertPhotoLayout.openPhotoViewer(photoEntry, true, false);
            return false;
...
//
                        ...
                        Log.d("TAG", "navigateTo(PAGE_PREVIEW11)");
                        chatAttachAlertPhotoLayout.openPhotoViewer(photoEntry, true, false);
                    }
                    ...
                    Log.d("TAG", "navigateTo(PAGE_PREVIEW12)");
                    MediaController.PhotoEntry photoEntry = new MediaController.PhotoEntry(0, ChatAttachAlertPhotoLayout.lastImageId, 0, outputFile.getAbsolutePath(), 0, outputEntry.isVideo, outputEntry.width, outputEntry.height, 0);
                    photoEntry.canDeleteAfter = true;
                    chatAttachAlertPhotoLayout.openPhotoViewer(photoEntry, true, false);
                } else {
...
//            animatePhotoFilterTexture(true, animated);
            previewTouchable = (Touchable) photoFilterView;
...
//            animatePhotoFilterTexture(false, animated);
        ...
        if (editMode == EDIT_MODE_PAINT) {
            createPhotoPaintView();
            previewTouchable = (Touchable) paintView;
...
//            rightMargin = dp(46);
    ...
    public void hidePhotoPaintView() {
        if (paintView == null) {
            return;
        }
        previewTouchable = null;
...
//        paintView.keyboardNotifier.ignore(true);
    ...
    public void createPhotoPaintView() {
        Log.d("TAG", "createPhotoPaintView");
        if (paintView != null) {
...
//            privacySelector.setVisibility(View.GONE);
    ...
    private void applyPaintInBackground(Runnable whenDone) {
        final PaintView2 paintView = this.paintView;
...
//        collageLayoutView.getLast().addView(cameraView, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, LayoutHelper.MATCH_PARENT, Gravity.FILL));
    ...
    private void requestCameraPermission(boolean force) {
        ...
        if (!noCameraPermission) {
            Log.d("TAG", "RequestCameraPermission_SR");
...
//            requestedCameraPermission = false;
    ...
    private void onRequestPermissionsResultInternal(int requestCode, String[] permissions, int[] grantResults) {
        ...
        if (requestCode == 111) {
            ...
            if (granted && currentPage == PAGE_CAMERA) {
                ...
                Log.d("TAG", "OnRequestPermissionsResultInternal_SR");
                if (CameraController.getInstance().isCameraInitied()) {
                    createCameraView();
                } else {
                    CameraController.getInstance().initCamera(this::createCameraView);
    ...
    private void openPremium() {
        ...
        PremiumFeatureBottomSheet sheet = new PremiumFeatureBottomSheet(new BaseFragment() {
            { currentAccount = StoryRecorder2.this.currentAccount; }
            @Override
            ...
            @Override
            public Activity getParentActivity() {
                return StoryRecorder2.this.activity;
            }
    ...
    public StoryRecorder2 selectedPeerId(long dialogId) {
    ...
    public StoryRecorder2 canChangePeer(boolean b) {
        canChangePeer = b;
        return this;
    }
